...
//
void buildWallsFromOutline(const PolyLine2f &outline, const float roofHeight, vector<Vec3f> &verts, vector<uint32_t> &indices)
{
...
// Using this for the walls in the roof (like the shed roof)
void buildWallsFromOutlineAndTopOffsets(const PolyLine2f &outline, const float bottomHeight, const OffsetMap &topOffsets, vector<Vec3f> &verts, vector<uint32_t> &indices)
{
    uint32_t base = verts.size();
    for ( auto i = outline.begin(); i != outline.end(); ++i ) {
        auto it = topOffsets.find( std::make_pair( i->x, i->y ) );
        Vec3f offset = ( it == topOffsets.end() ) ? Vec3f::zero() : it->second;
        Vec3f bottom = Vec3f( i->x, i->y, bottomHeight );
        verts.push_back( bottom );
        verts.push_back( bottom + offset );
    }
    uint16_t totalVerts = verts.size();
...
void buildRoofFaceFromOutlineAndOffsets( const PolyLine2f &outline, const OffsetMap &offsets, const float roofHeight, vector<Vec3f> &verts, vector<uint32_t> &indices )
{
...
void buildRoofFromSkeletonAndOffsets( const SsPtr &skel, const OffsetMap &offsets, const float roofHeight, vector<Vec3f> &verts, vector<uint32_t> &indices )
{
    for( auto face = skel->faces_begin(); face != skel->faces_end(); ++face ) {
        ...
        buildRoofFaceFromOutlineAndOffsets( faceOutline, offsets, roofHeight, verts, indices );
    }
}
...
{
    OffsetMap empty;
    buildRoofFaceFromOutlineAndOffsets( outline, empty, roofHeight, verts, indices );
}
...
void buildHippedRoof(const PolyLine2f &outline, const float roofHeight, vector<Vec3f> &verts, vector<uint32_t> &indices)
{
    ...
    // - triangulate roof faces and add to mesh
    buildRoofFromSkeletonAndOffsets(skel, offsetMap, roofHeight, verts, indices);
}
...
void buildShedRoof(const PolyLine2f &outline, const float roofHeight, const float slope, vector<Vec3f> &verts, vector<uint32_t> &indices)
{
    ...
    std::map<std::pair<float, float>, Vec3f> offsetMap;
    for (auto i = outline.begin(); i != outline.end(); ++i) {
    ...
    // - triangulate roof faces and add to mesh
    buildRoofFaceFromOutlineAndOffsets( outline, offsetMap, roofHeight, verts, indices );
    buildWallsFromOutlineAndTopOffsets( outline, roofHeight, offsetMap, verts, indices );
}
...
gl::VboMeshRef BuildingPlan::makeMesh()
{
    ...
    buildWallsFromOutline(mOutline, roofHeight, verts, indices);
    ...
    switch ( mRoof ) {
        ...
        case SHED_ROOF:
            ...
            buildShedRoof(mOutline, roofHeight, 0.2, verts, indices);
            break;
        case GAMBREL_ROOF:
